...
const historyKey = (value: string) => value.replace(/\s+/g, "").toLocaleLowerCase();
...
export function Materials({ products, act }: { products: Product[]; act: (work: () => Promise<unknown>, success: string) => Promise<boolean> }) {
  ...
  const categoryKey = historyKey(productCategoryInput);
  const categoryProducts = activeProducts.filter(item => historyKey(item.category_name || "") === categoryKey);
  ...
  return <section className="human-page material-classification-page">
    ...
    <div className="human-card full"><div className="human-card-title"><div><h2>选择产品与视频</h2><p>产品分类和产品名称可以直接输入；成功归类后自动保留为历史。</p></div></div>
      <div className="classification-master-inputs">
        <label>产品分类<div className="classification-history-field"><ClassificationDropdown value={productCategoryInput} options={productCategories} onChange={value => { setProductCategoryInput(value); setProductInput(""); }} onSelect={item => { setProductCategoryInput(item.name); setProductInput(""); }} placeholder="输入产品分类，或选择最近记录" /></div><small>展开显示最近 5 次输入；输入文字可模糊查询历史</small></label>
        <label>产品名称<div className="classification-history-field"><ClassificationDropdown value={productInput} options={categoryProducts} disabled={!productCategoryInput.trim()} onChange={setProductInput} onSelect={item => setProductInput(item.name)} placeholder={productCategoryInput.trim() ? "输入产品名称，或选择最近记录" : "请先输入产品分类"} /></div><small>新名称会在本次归类成功后自动保存</small></label>
    ...
    <div className="human-card full"><div className="human-card-title"><h2>逐个视频打标签</h2><span>像背景音乐一样自由输入多个标签，使用逗号分隔</span></div>
      ...
      {tagEditor && <div className="confirm-dialog-backdrop" role="presentation"><section className="confirm-dialog music-tag-dialog classification-tag-dialog" role="dialog" aria-modal="true" aria-labelledby="classification-tag-title"><div><b id="classification-tag-title">设置视频标签</b><span>“{tagEditor.name}”</span><label>视频标签<input autoFocus value={tagInput} onChange={event => setTagInput(event.target.value)} onKeyDown={event => { if (event.key === "Enter") { event.preventDefault(); saveVideoTags(); } }} placeholder="例如：正面，近景，手持展示" /><small className={tagInputError ? "field-error" : ""}>{tagInputError || "多个标签使用逗号分隔，清空后保存可移除全部标签。"}</small></label></div><div><button type="button" className="human-secondary" onClick={() => setTagEditor(null)}>取消</button><button type="button" className="music-tag-confirm" disabled={Boolean(tagInputError)} onClick={saveVideoTags}>保存标签</button></div></section></div>}
    </div>
  </section>;
}
