...
type DraftLibraryDetail = { title: string; content: string; meta: string };
...
export function DraftProduction({ copies, narrations, music, drafts, act }: {
  ...
  const [libraryDetail, setLibraryDetail] = useState<DraftLibraryDetail | null>(null);
  ...
  return <section className="human-page draft-production-page">
    ...
    {libraryDetail && <div className="confirm-dialog-backdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target) setLibraryDetail(null); }}><section className="confirm-dialog draft-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="draft-detail-title"><div className="confirm-dialog-body"><b id="draft-detail-title">{libraryDetail.title}</b><span>{libraryDetail.meta}</span><div className="draft-detail-content">{libraryDetail.content}</div></div><div className="confirm-dialog-actions"><button type="button" onClick={() => setLibraryDetail(null)}>关闭</button></div></section></div>}
  </section>;
}
